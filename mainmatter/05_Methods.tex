...
3 points were obtained using projected speckles and they are summarized in Table \ref{tab: projected speckle matrix} as points 29-31. A $f_\#$ sweep was done to assess the effect of that variable with a $d_{GG}$ of 300mm, as it was the distance where the most amount of points were collected for the directed speckle case. Therefore, a direct comparison can be done between the results obtained with projected speckles and direct speckles.
...
\begin{table}[htbp]
    ...
    numbering continues from it, so $f_\#$ is the actual f-stop number rather than
    the marked one, $d_{GG}$ is the distance between the beam expander lens and the
    projection plane, $M_{ag}$ the optical magnification obtained from $d_f$ through
    the thin lens relation of Equation \ref{equation: M} and $\Delta s_{th}$ the
    speckle size predicted by the Rayleigh criterion (Equation \ref{equation: Rayleigh Limit}).
