...
\par In that reference the mean speckle size is defined as the lag at which the normalized autocorrelation function of the intensity decreases to $1/e$, and it was evaluated here with the one-dimensional correlation method of a long vector, where all the rows or all the columns of the image are concatenated into a single vector before the correlation is computed. It is worth noting that the size defined in this manner corresponds to a half width of the correlation peak, therefore the full width was adopted here, so that the reported quantity remains comparable with the speckle size predicted by Equation \ref{equation: Rayleigh Limit}.

\par However, the recorded intensity does not contain the speckle fluctuation alone, since the expanded beam illuminates the field of view in a non-uniform manner. That illumination envelope is correlated over hundreds of pixels, therefore it adds a positive pedestal to the autocorrelation function, which does not decay within the range of lags of interest and biases the measured size towards larger values. Consequently, each image was normalized by a Gaussian low-pass filtered copy of itself before the correlation was computed, a pre-processing step that is standard practice in speckle correlation imaging \cite{Shi2023,Zhu2025}. The width of the filter was selected such that the autocorrelation of the normalized image vanishes at lags well beyond the correlation peak, since a narrower filter could attenuate the speckle itself, whereas a wider one would leave part of the envelope in the image.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrrr}
        ...
        11 &  4.00 & 300 & 0.4167 & 680 & 0.67 \\
        12 & 11.31 & 300 & 0.4167 & 680 & 1.90 \\
        13 & 22.63 & 300 & 0.4167 & 680 & 3.81 \\
        \midrule
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrrr}
        ...
        Point & $f_\#$ & $d_{GG}$ & $M_{ag}$ & $d_f$ & $\Delta s_{th}$ \\
              & [--]   & [mm]     & [--] & [mm] & [px] \\
        \midrule
        29 &  4.00 & 300 & 0.8511 & 435 & 0.88 \\
        30 & 11.31 & 300 & 0.8511 & 435 & 2.49 \\
        31 & 22.63 & 300 & 0.8511 & 435 & 4.97 \\
        \bottomrule
    \end{tabular}
\end{table}
