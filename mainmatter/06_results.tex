...
Using S ratio method, the sensitivity and effective lever arm of points 8-11 can be obtained with the maximum measured displacement of points 4-7. 


\begin{table}[htbp]
    \centering
    \caption{Sensitivity of points 8 to 11 obtained with the ratio method of Equation \ref{equation: S ratio}, using the maximum displacements of points 4 to 7. $\Delta_{max}$ is the maximum displacement measured at each point and $S = l \cdot M_{ag}$ the geometric sensitivity, which is not defined for the points with the focus inside the test section. Avg is the average of the sensitivities obtained by the ratio method and $l_{eff} = S_{avg}/M_{ag}$ the effective lever arm that follows from it}
