...
Similar as before, the peaks of points with the same flow condition were compared to verify if both density gradients match as they should. This calculation is displayed in Table \ref{}. Once again, the results are remarkably similar. The biggest deviation happened between points 4 and 8 in the peak of the y component of the density gradient, correspondent to 17,3\%. Nevertheless, this can be considered an outlier as the average of the 100 highest values of the two points showed an error below 10\%. The data in the table verifies the resultant density gradient fields by showing that two different routes will yield the same result. Another interesting observation is that points 4 and 8, that imaged an $NPR = 8$, showed considerably similar peaks and peak averages to points 1,2 and 3 which had the same $NPR$.

%Add peak comparions of the external flow points
