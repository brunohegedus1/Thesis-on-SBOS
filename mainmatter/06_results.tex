...
\begin{table}[htbp]
    \centering
    \caption{Speckle sizes measured during the angled glass campaign, for the configurations of Table \ref{tab: angled glass test matrix}. $M_{ag_{exp}}$ is the magnification measured with the calibration plate, $\Delta s_{th}$ the size predicted by the Rayleigh criterion of Equation \ref{equation: Rayleigh Limit} and $\Delta s_{exp}$ the size returned by the DaVis estimator. The last column is the difference between the two. The columns $D_s$ give the diameter of the speckle pattern on the sensor, predicted as $D \cdot M_{ag}$ and measured from the image. Points 11 to 20 of Table \ref{tab: angled glass test matrix} repeat the optical layout of point 10 and are therefore omitted.}
