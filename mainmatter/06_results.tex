...
\begin{table}[htbp]
    \centering
    \caption{Speckle sizes measured with directed speckles, for the configurations of Table \ref{tab: speckle investigation matrix}. $M_{ag_{exp}}$ is the magnification measured with the calibration plate, $\Delta s_{th}$ the size predicted by the Rayleigh criterion and $\Delta s_{exp}$ the measured size, reported both by the DaVis estimator and by the one-dimensional autocorrelation method of Hamarová et al. \cite{Hamarova2016}. The deviation columns give the difference of each measurement from the prediction. DaVis returned no value for point 10. The column $u_{tot}$ is the combined standard uncertainty of $\Delta s_{exp}$ obtained by the autocorrelation method, and it does not apply to the DaVis value. It combines in quadrature the contribution of the flattening filter, computed for each point as the sensitivity of the measured size to the filter width multiplied by the range of widths for which the autocorrelation tail remains within its noise floor, and the spread between the estimators reported by Hamarová et al. \cite{Hamarova2016}. Points 1 to 13 vary the aperture and the ground glass distance, whereas points 14 to 28 hold both fixed and sweep the magnification.}
