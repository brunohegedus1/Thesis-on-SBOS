...
The various $f_\#$ sweeps made during the experimental campaign allows to evaluate if the interference waves became diffraction limited at the pupil. Figure \ref{} shows the trend of average speckle size with f-stop number for 4 different configurations. All lines follow the characteristic increasing trend of a developed subjective speckle, which follows equation \ref{equation: Rayleigh Limit}, and signals that the interference front is becoming diffraction limited at the lens pupil \cite{Goodman2020}. The red and yellow lines both share the same $d_{GG}$ what makes them viable for direct comparison. 

Through them, it is visible that the magnification is also playing a part in the speckle size. A comparison with trends from different $d_{GG}$ is also reasonable as it will be demonstrated later. Comparing the last two lines with the purple one, it is again possible to observe the same trends, with speckle increasing not only with $f_\#$ but also with $M_{ag}$. In green, the projected speckle trend can also be analyzed. It demonstrates that projected speckles are generally smaller, presenting smaller speckles even to yellow which has a lower magnification than green. The Figure demonstrates the, that despite large deviations compared to the Rayleigh limit the results shows roughly the expected trends.

%add f-stop sweeps

By using the points where only $d_{GG}$ was varied, it is possible to check the influence of the beam diameter in the speckle generation. Figure \ref{} shows that the average speckle size remains quite consistent for high $d_{GG}$, as it varies from 2.91 px at 190mm, dips to 2.29 px at 450 mm and goes to 2.71 px at 740mm. However it spikes at low $d_{GG}$ when the beam diameter is small, reaching 4.59 px at 100mm with beam ratio of 1.25. This data supports the hypothesis that the larger speckle size in the angled glass experiment was caused by the small diameter of the laser when it hit the 12.7cm ground glass diffuser. The fact that the speckle barely changed for larger distances is also backed up by the literature, which doesn't mention the effect of the laser diameter in the speckle generation. In addition, it means that, when designing the laser optical setup, an experimentalist must only worry about expanding the laser beam to fill the desired field of view and make sure it is large enough.
