...
% REVIEW: point 1 is called an outlier because no cause was found. Absence of an
% REVIEW: explanation is not what defines an outlier. The commented-out alternative
% REVIEW: paragraph below this one is stronger: it quantifies point 1 at 2.5 times
% REVIEW: u_tot and tests the glass-angle hypothesis. Consider using it instead.
The table allows the comparison of the measurement error with its inherent uncertainty. In general terms, the error obtained is below the combined uncertainty, with the exception of points 1, 17 and 18. Only point 1, however, differs significantly from the uncertainty, with a difference of 4.76 percentage points, while 17 and 18 deviate by less than 0.5 percentage points. Regarding point 1, even the optical magnification and $d_f$ error shown in Table \ref{tab: df and Mag angled glass results} do not stand out compared to the other points, meaning that the design values alone do not reveal the source of that deviation. Therefore, point 1 can be considered an outlier. The slider experimental error is also worth a remark due to the distinct behavior of its deviation. As it can be seen in Table \ref{tab: angled glass displacement results}, all the deviations of the slider experiments are positive and have a similar magnitude, which leads to the interpretation of an induced bias in the measurement. It is not clear what caused this bias, although it was probably introduced by an added error in the magnification and glass angle, as both were fixed throughout the measurements.
...
% REVIEW: two claims in this paragraph need rewording.
% REVIEW: (a) 'the S_geom error is almost the same as the magnification error' is an
% REVIEW:     identity, not an observation: S_geom = l M_exp and S_th = l M_th share
% REVIEW:     the same l, so the relative deviation IS the magnification deviation.
% REVIEW: (b) S_Delta is not independent either. Its deviation reproduces
% REVIEW:     (1+e_dx)(1+e_M)-1 for all nine points to within rounding. Table 6.3 is
% REVIEW:     therefore a re-expression of Tables 6.1 and 6.2, not a second route.
% REVIEW: (c) 'most points are below u_tot or quite close to it' understates the
% REVIEW:     result: 10 of 19 points exceed u_tot (1, 2, 7, 9, 13, 14, 15, 18, 19,
% REVIEW:     20). Points 13-15 and 18-20 exceed by under one percentage point and
% REVIEW:     are exactly the slider points carrying the systematic offset named
% REVIEW:     earlier, which is worth saying explicitly.
The data collected during the experimental campaign allows the assessment of sensitivity through two experimental routes, each one providing a different insight on the error sources and validity of the experiment. Table \ref{tab: sensitivity comparison} compares the theoretical sensitivity given by the surface map of Chapter \ref{chapter: methodology} to the sensitivity obtained through the geometry $S_{geom} = l \cdot M_{ag_{exp}}$ and the sensitivity obtained with the displacement data $S_\Delta = \Delta_x \cdot l_{px}/\varepsilon_{glass}$. With the exception of point 7, which has already been discussed, all data is within 8\% of agreement. It is worth noting that the $S_{geom}$ error is almost the same as the magnification error, while $S_\Delta$ stacks both the displacement measurement error and the geometric setup error. The total combined uncertainty of $S_\Delta$ has also been calculated and is displayed in the last column, most points are below $u_{tot}$ in magnitude or quite close to it with the exception of point 7 and 9. Nevertheless, the data shows that the optical configuration parameters defined by the methodology of Chapter \ref{chapter: methodology} produce a setup with the predicted sensitivity within an acceptable error range. Therefore, it validates the methodology as it demonstrates that the physical setup reproduces the predicted performance.
...
% REVIEW: because it also folds in u_glass. Different quantities, same promise.
    ...
    \begin{tabular}{llrrr}
        ...
        \midrule
        Combined, $u_{\Delta x} = \sqrt{\sigma_{rel}^2 + c_{M_{ag}}^2 }$             &  &  &  & 2.01 to 6.04 \\
...
% REVIEW: the error bars are described as the standard deviation divided by the mean
% REVIEW: displacement, a dimensionless ratio, on an axis in mm. Presumably that
% REVIEW: fraction times S.
...
% REVIEW: CHECK THE RAW DATA FOR POINTS 5 AND 6 OF TABLE 6.6 BEFORE WRITING MORE.
% REVIEW: Sorting Delta s exp by defocus type splits almost perfectly:
% REVIEW:   Negative: pt1 27.61  pt3 29.29  pt5 11.39  pt7 34.35
% REVIEW:   Positive: pt2 11.81  pt4  8.73  pt6 28.20  pt8  8.81  pt9 10.31
% REVIEW: Points 5 and 6 are the only exceptions. They are adjacent rows with the
% REVIEW: same f_# = 11.31 and the same l = 80 mm. Exchanging those two values makes
% REVIEW: the separation exact and raises corr(M_ag, Delta s exp) from +0.67 to
% REVIEW: +0.97. Two adjacent rows being the sole counterexample to two separate
% REVIEW: trends looks like a transcription swap.
% REVIEW: Caveat if it is confirmed: negative defocus means a shorter d_f and so a
% REVIEW: higher M_ag in this campaign, so defocus sign and magnification are
% REVIEW: confounded and cannot be separated from these points.
Section \ref{section: Angled Glass Experiment} mentioned a discrepancy between the predicted average speckle size obtained through the Rayleigh diffraction limit and the ones obtained experimentally and computed using DaVis. Table \ref{tab: angled glass speckle results} displays both quantities side by side together with the $f_\#$ and $M_{ag}$ used at each point. The $\Delta s$ deviation shows that at some points the discrepancy exceeds the theoretical value several times over. Speckle formation is an intrinsically random phenomenon, described by complex physics \cite{Goodman2020, NikitaFominBook, Cloud2007}, so it is not exactly clear what is precisely the root of this difference. Nevertheless, the data gives some clues on what could be improved.

% REVIEW: the magnification claim is weakly supported as the table stands:
% REVIEW: corr(M_ag, Delta s exp) = +0.67, with two counterexamples in the same
% REVIEW: table (pt5 M = 1.043 -> 11.39 px, pt6 M = 0.718 -> 28.20 px). See the note
% REVIEW: above about a possible pt5/pt6 swap.
% REVIEW: Cleaner f_# evidence than points 8-10, which also vary M: point 4 at
% REVIEW: f_# 16 gives 8.73 px and point 8 at f_# 4 gives 8.81 px. Four times the
% REVIEW: aperture, no change at all.
% REVIEW: Also worth reporting: Delta s exp is bimodal, clustering at 8.7-11.8 px
% REVIEW: and at 27.6-34.4 px with nothing in between.
From points 8-10, the $f_\#$ varies significantly. However, little to no effect is observed in the average speckle size. In contrast, changes in magnification vary the experimental average speckle size considerably. These factors lead to the belief that the pattern did not become diffraction limited at the lens pupil but rather at the ground glass itself, so what is imaged are the phase cells present at the diffuser.

% REVIEW: 'follows very closely' is generous. Seven of nine points agree within
% REVIEW: 17 %, but points 1 and 3 are +50 % and +59 %.
Moreover, the speckle pattern diameter $D_{s_{exp}}$ follows very closely the same trend as $D_{s_{th}}$ which is the size of the image of a collimated source, computed as $D_{s_{th}} = M_{ag} \cdot D_{ap}$. This could mean that the scattering cone coming out of the ground glass diffuser is too narrow and far from the lens, so that it behaves almost as a collimated source when it reaches the aperture. The difference between both values is then explained by the remaining scattering of the laser.

% REVIEW: three problems in this paragraph.
% REVIEW: (a) two different meanings of 'Rayleigh' are welded together. Goodman's
% REVIEW:     Rayleigh is the amplitude STATISTICS of a developed speckle field. The
% REVIEW:     Rayleigh of Equation 4.9 is a diffraction LIMIT, a resolution
% REVIEW:     criterion. 'Rayleigh diffraction limited distribution' merges the two.
% REVIEW:     This is the load-bearing step of the conjecture.
% REVIEW: (b) the cited passage is about a projector illuminating a screen, not
% REVIEW:     about imaging a diffuser. The commented note below already calls this
% REVIEW:     a stretch. Cite the objective/subjective speckle distinction directly.
% REVIEW: (c) the overfill conjecture is checkable and looks doubtful. Filling the
% REVIEW:     pupil needs a scattering half-angle of about arctan((D/2)/z); with
% REVIEW:     D = 12.5 mm and z of order 400 mm that is roughly 0.9 deg, and a ground
% REVIEW:     glass scatters over tens of degrees.
% REVIEW:     A free-space (objective) speckle explanation does not close either:
% REVIEW:     with the 12.7 mm illuminated diffuser of Chapter 5 and z = 300 mm,
% REVIEW:     lambda z / D = 15 um, about 2.3 px at M = 1, not the 27 px measured.
% REVIEW:     What survives is the explanation already given above, that the camera
% REVIEW:     resolves and images the diffuser's own phase cells; at M = 1 the
% REVIEW:     measured 27.61 px corresponds to a 179 um cell, plausible for a coarse
% REVIEW:     ground glass.
% REVIEW:     One more candidate never considered: Chapter 5 sets the DaVis sliding
% REVIEW:     average and minimum to 16 px, while Delta s th ranges 0.75-3.90 px. A
% REVIEW:     16 px kernel cannot report a 1 px speckle, and the measured floor sits
% REVIEW:     at 8.7-11.8 px.
A property of speckles mentioned by Goodman \cite{Goodman2020} might help to understand the phenomena observed during the campaign. In his book he says that when the scattering cone does not overfill a lens, the image captured corresponds only to random phase cells of the diffuser, whereas if the lens is overfilled, a Rayleigh distribution is produced. Putting all evidence together results in the following conjecture: the collimated laser light hitting a small diffuser created a scatter cone that was too narrow to overfill the lens. Consequently, a Rayleigh diffraction limited distribution was not created at the pupil and the camera could only image the random phase cells of the diffuser.

%Hypothesis: too narrow of a scatter cone can't overfill the lens, thus the lens only images the random phase cells of the diffuser and do not create a Rayleigh diffraction limit distribution . Good. Now how can I write this on the previous paragraph?
...
\begin{table}[htbp]
    \centering
% REVIEW: two things to settle in this table.
% REVIEW: (a) the caption points at Table 5.1, which lists MARKED f-numbers (11, 16)
% REVIEW:     while this table lists actual ones (11.31, 16.00).
% REVIEW: (b) point 5 carries '>3336' with no explanation anywhere.
    \caption{Speckle sizes measured during the angled glass campaign, for the configurations of Table \ref{tab: angled glass test matrix}. $M_{ag_{exp}}$ is the magnification measured with the calibration plate, $\Delta s_{th}$ the size predicted by the Rayleigh limit of Equation \ref{equation: Rayleigh Limit} and $\Delta s_{exp}$ the size returned by the DaVis estimator. The last column is the difference between the two. The columns $D_s$ give the diameter of the speckle pattern on the sensor, predicted as $D \cdot M_{ag}$ and measured from the image. Points 11 to 20 share the optical layout of point 10, so their predicted and measured sizes are identical and have been omitted.}
    \label{tab: angled glass speckle results}
    \begin{tabular}{lrrrrrrr}
        \toprule
        Point & $f_\#$ & $M_{ag_{exp}}$ & $D_{s_{th}}$ & $D_{s_{exp}}$ & $\Delta s_{th}$ & $\Delta s_{exp}$ & $\Delta s$ dev. \\
              & [--] & [--] & [px] & [px] & [px] & [px] & [px] \\
        \midrule
         1 & 16.00 & 1.002 & 1839 &    2750 &  3.72 &  27.61 & $+23.89$ \\
         2 & 16.00 & 0.688 & 1261 &    1196 &  3.15 &  11.81 & $+8.67$ \\
         3 & 16.00 & 1.045 & 2019 &    3209 &  3.90 &  29.29 & $+25.39$ \\
         4 & 16.00 & 0.671 & 1237 &    1119 &  3.12 &   8.73 & $+5.61$ \\
         5 & 11.31 & 1.043 & 2900 & $>3336$ &  2.74 &  11.39 & $+8.65$ \\
         6 & 11.31 & 0.718 & 1929 &    1878 &  2.27 &  28.20 & $+25.93$ \\
         7 & 11.31 & 1.054 & 1836 &    1530 &  3.02 &  34.35 & $+31.33$ \\
         8 &  4.00 & 0.572 & 2355 &    2363 &  0.75 &   8.81 & $+8.06$ \\
         9 & 16.00 & 0.640 & 1175 &    1130 &  3.06 &  10.31 & $+7.25$ \\
        10 &  8.00 & 0.746 & 2849 &    2640 &  1.65 &   9.70 & $+8.04$ \\
        \bottomrule
    \end{tabular}
\end{table}
