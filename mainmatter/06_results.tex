...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{SBOS results for a compressible jet. Displacement magnitude fields obtained through cross correlation for air tap gauge pressures from 3 bar to 8 bar with a step of 1 bar. Respective Nozzle Pressure ratio from the gauge pressures are obtained as $NPR_g = (p_g+p_a)/p_a$. All fields share the same color scale and masked vectors are shown in grey}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrrrrr}
        \toprule
        Data & $\nu$ & $M_j$ & $u_{M_j}$ & $NPR$ & $u_{NPR}$ & $p_e$ & $u_{p_0}$ \\
             & [$^{\circ}$] & [-] & [\%] & [-] & [\%] & [bar] & [\%] \\
        \midrule
...
The fitted curves using Emden's and Hartmann-Lazarus' formulations are displayed together with the literature solutions in Figure \ref{fig: shock cell lengths}. The obtained values for the constants are: $k = 1.05, a = 0.894$ and $b = 0.28$. The $k$ obtained matches exceptionally well the value obtained by Hartmann and Lazarus \cite{Hartmann1941}, whom achieved $k = 1.06$. However, it does differ from the estimates from Emden \cite{Emden1899}. This deviation might be due to the difference in which he defined $L_s$ as the weighted averaged shock cell length, whereas in this case the same methodology of H-L was used where all cell lengths after the first were averaged. It might be also due to differences in the nozzle shape itself. The $a$ and $b$ values also are remarkably consistent with H-L values, namely $a = 0.98$ and $b = 0.3$. Visually, the data is positioned in-between Hartmann-Lazarus' equation for the first cell, follows almost perfectly the fit of H-L's average and above Emden's weighted average formulation. Despite the difference to Emden's equation, the data follows closely both of Hartmann-Lazarus' results, which is a favorable sign of the accuracy of the $NPR$ estimates and $L_s$ sizes.

The raw data and density gradients allowed for a preliminary study of the compressible jet properties. The two allowed to identify the anomaly in $NPR_g$ estimated from the wall air tap gauge pressure and recalculate a new estimate based on the shear layer angle at the nozzle lip. Then the planar density gradients allowed to identify the jet's structure, specially the shock cell length. The latter was then correlated to the corrected $NPR$ following two literature's methodologies. The comparison of this thesis' results with the ones obtained by Emden and Hartmann-Lazarus indicates that the $NPR$ estimate is closely related to literature's values and validates the estimate. The next section will build on these values to compare and validate the processes and algorithms implemented to obtain the density field of the imaged jets. A discussion of such methods and its results will follow next.

\subsection{Quantitative Density Field}
\label{subsection: Quantitative Density Field results}

One of the goals of the compressible jet experiment was the quantitative reproduction of its density field using the displacement data gathered with SBOS. To achieve that, the vector field had to go through a series of post-processing steps, integration schemes and noise regularization. The first ones were already described: the median of the background vector was removed and the vectors were translated and rotated to position the jet axis horizontally at n = 0. The two halves of the field were then folded about $n = 0$ and averaged. The streamwise displacement is an even function of $n$, so the two halves were summed and halved, while the normal displacement is an odd function, so the lower half was multiplied by $-1$ before the average was taken.The correct positioning of the axis is essential for this step and for the Abel inversion, as both assume an axissymmetric field and any misalignment induce errors by mixing the two halves of the jet.
...
Through analysis of Figure \ref{fig: centerline density e vs panda}, dataset e can be compared with the data from literature to validate the density field results. The plot shows that the SBOS data fits incredibly well the first valley and peak values of Panda and Seasholtz but it starts to drift from the second cell downstream, where the literature shock cells shrink and decay in amplitude a lot faster. The jet length of the SBOS measurements have already been validated with the literature data from Hartmann-Lazarus, so another effect most be playing a part. It was conjectured that this discrepancy can be explained by the size difference between the P-S nozzle and the one employed in this experiment. The bigger jet disturbs more air, causing turbulence and a thicker shear layer, which increases the jet energy dissipation with ambient air and accelerates the decay of the shock cell length, it's the Reynold's effect. This effect is smaller at the beginning of the jet and increases with length, therefore, the match of the first cell is enough to validate the results obtained. Nevertheless, it is worth to further explain the discrepancy between the two plots.

The Reynolds number can be used to assess the development of a boundary layer and infer if the flow is turbulent or laminar. The number described in Equation \ref{equation: Reynolds Number} is proportional to the nozzle diameter $d_N$, velocity $V$, density $\rho$ and viscosity $\mu$. Using the fully expanded values obtained with isentropic relations, it is possible to compare the Reynolds number of this work with the one of Panda and Seasholtz. Since $M_j$ is the same, the Reynolds number scales with only $d_N$. For this work's $d_N = 3.5$mm and $M_j = 1.43$, Re = $1.78 \cdot 10^5$, while for the $d_N = 25.4$mm of Panda and Seasholtz, $Re = 1.29 \cdot 10^6$. The 10 times difference matters a lot in this regime as $Re = 10^5$ marks the transition between a partially laminar and a fully turbulent jet \cite{Ozawa2020}.
...
The jets in the transition regime between laminar and turbulent jets experience large differences in the boundary layer shape, as investigated by Ozawa et al. \cite{Ozawa2020}. Their figure 12 shows that, for larger Reynolds number (Re = $10^6$), the velocity along the centerline decay faster, while it stays more persistently high at low Reynolds ($Re = 10^5$), meaning that higher Reynolds effects do indeed increase decay downstream. Further literature agrees with these findings. Seiner and Norum \cite{Seiner1980} have reported that the spread of the viscous mixing layer shifts the shock positions predicted by inviscid solutions and weakens the shocks strength further downstream. Moreover, the fact that Hartmann-Lazarus formulation matched this work's data can also be explained by Reynolds similarity as the range of $M_j$ was similar ($NPR$ around 4) and their nozzle diameter ranged from 4mm to 6mm, yelding a $Re \approx 2.7-4.0 \cdot 10^5$ \cite{Hartmann1941}.

The compressible jet experiments allowed the imaging of a real schlieren object using Laser-Speckled Background Oriented Schlieren, resulting in a displacement field which was later processed into quantitative density data. The CC results yielded remarkable spatial resolution, which made possible the identification of the small jet's shock cell structural features and the comparison between different $NPR$s. The shock cell lengths were compared to literature data from Hartmann-Lazarus and Emden \cite{Hartmann1941, Emden1899, Powell2010}, validating the $NPR$ estimates made by measuring the shear layer angle at the nozzle lip. Abel inversion with Tikhonov regularization was then implemented to obtain quantitative density fields of the jets. The density results were validated against the centerline density data of Panda and Seasholtz \cite{Panda1999}. The unusual small diameter of the nozzle used also permitted the visualization of Reynolds effects when compared to the larger exit diameters common in the study of underexpanded jets. Therefore, the experiment shows that SBOS can be used to image small flows with impressive resolution and is able to extract meaningful quantitative data using a considerable simple setup. 
