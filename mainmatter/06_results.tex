...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/aerospike displacement point 1.png}
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/aerospike displacement point 2.png}
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/aerospike displacement point 3.png}
        \caption{Point 3, positive defocus}
        \label{fig: aerospike displacement point 3}
    \end{subfigure}
    \caption{Displacement magnitude fields of the aerospike nozzle exhaust at $NPR = 8$ without freestream flow, obtained through cross-correlation for the three defocus configurations. The fields were rotated so that the top edge of the processed region is horizontal and cropped to a common aspect ratio, which removes the top and bottom edges of the field of point 2 and the downstream end of point 3. Each field uses its own color scale and masked regions are shown in grey}
    \label{fig: aerospike displacement points 1-3}
\end{figure}

Figure \ref{fig: aerospike flow pictures} displays the flow pictures taken for points 1 on the left, 3 on the right and an schlieren image of the same flow condition in the middle, which focus inside the test section. The SBOS flow pictures are notably blurred, with almost no distinct speckle inside the flow. This blur probably caused decorrelation, which showed up in the results as noise. The blur cannot be only attributed to a defocus distance and seems to come from the flow itself. This is hinted by the schlieren image that, despite being sharper than the other two, still shows a very darkened and unclear flow. This predominance of dark regions means that the light was bent severely and is being blocked by the horizontal filter, indicating a large density gradient in those regions. This analysis shows a limitation of the defocus configuration but also an interesting feature of SBOS. It indicates that the defocused method might fail in flows with very strong density gradients, however, the technique allows focusing inside the test section, dynamically changing the sensitivity and resolution so that the blur can be reduced and the flow studied successfully.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.277\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/aerospike flow picture point 1.jpg}
...
The maximum measured displacement of points 1,2,3 were taken from the Cross-Correlation result and displayed in Table \ref{tab: S ratio}. The sensitivity of point 1 was calculated based on the one of point 3 and vice-versa. Then, the sensitivity of point 2 was computed based on points 1 and 3. The deviation column shows that this method matches remarkably well the  $S= l \cdot M_{ag}$ formulation for the two defocused points. For the point inside the text section, the sensitivity estimate using either defocused point also yield similar results with less than 5\% of deviation. The deviations between the two sensitivity estimates might come from differences in the schlieren object, which affect $\varepsilon$ and $\Delta_{max}$ or errors in the magnification and distance measurements. Nevertheless, this small demonstration shows the simplicity and promise of this method, enabling SBOS images to be taken with the refractive object in focus.
