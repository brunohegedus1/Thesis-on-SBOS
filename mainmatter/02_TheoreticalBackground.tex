...
When a refractive object is axially symmetric, it is possible to reconstruct it's whole density distribution with only a 2D snapshot of the light displacements through it. In such scenario, the image is a 2D projection of a 3D axissymetric body, and the inverse Abel transform can be used to obtain it's radial and axial profile. Consider the case in Figure \ref{fig: Abel transform} and equation \ref{equation: displacement integral} for y, it can be integrated in y in both sides into equation \ref{}, which removes the partial derivative through the fundamental theorem of calculus.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{figures/AbelTransform.png}
    \caption{Diagram of the geometry of an axisymmetric body being seeing by an observer in I. If the body, in grey, is described by a function $f(r)$, the observer will see its projection $F(y)$, which is its Abel transform}
