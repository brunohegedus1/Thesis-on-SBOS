...
% REVIEW: the clause "to assess if the method has sufficient resolution and sensitivity" appears twice in consecutive sentences.
\par The flow through the ramp of an aerospike nozzle will be investigated later using SBOS  to assess if the method has sufficient resolution and sensitivity to correctly study such a complex flow field. In that context, the description of the flow physics will be of utmost importance to assess if the method has sufficient resolution and sensitivity to correctly study the application. It is now clear from the previous sections how high speeds and geometry lead to different flow structures that vary the fluid's density. The next section will explain how density influences the bending of light moving through a fluid, which is the fundamental principle behind optical density measurement techniques.
...
% REVIEW: $\varepsilon_1$ and $\varepsilon_2$ denote the Snell incidence and refraction angles above, while $\varepsilon_x$ here is the total deflection. Consider separate symbols.
